        ...
        public async Task<IActionResult> Index()
        {
            ...
            using (var httpClient = new HttpClient())
            {
                using (var cevap = await httpClient.GetAsync($"{uri}/api/Product/TumUrunleriGetir"))
                {
                    ...
                    urunler = urunler.Where(x => x.SupplierID == 1).ToList(); // Sadece İlgili tedarikçinin kendi ürünleri gelsin istedik.

                    //urunler = urunler.Where(x => x.SupplierID == Convert.ToInt32(HttpContent.User.FindFirts("ID").Value)).ToList();
                }
                return View(urunler);
            }
        }
